...
def generate_resume_content(data: dict) -> str:
    ...
    if data.get('job_description'):
        jd_context = f"""
        IMPORTANT: Optimize this resume for the following Job Description. 
        Use relevant keywords and tailor the content to match the requirements:
        ---
    ...
    chat_completion = client.chat.completions.create(
        messages=[
            {"role": "system", "content": "You are an expert professional resume writer and ATS optimization specialist."},
...
def generate_cover_letter(data: dict) -> str:
    jd_context = ""
    if data.get('job_description'):
        jd_context = f"Tailor this cover letter for the following job: {data['job_description'][:500]}..."
